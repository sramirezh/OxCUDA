...
int main( int argc, const char** argv) 
{
  ...
  float near_pow, resized_elements;
  // initialise card
  ...
  near_pow = ceil(log(num_elements)/log(2.0));
   
  resized_elements = pow(2.0,near_pow);  

  num_threads  = resized_elements;
  mem_size     = sizeof(float) * resized_elements;
  ...
  for(int i = 0; i < resized_elements; i++)
	if (i>=num_elements)
	{
	h_data[i]=0;
 	}
